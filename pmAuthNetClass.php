...
class pmAuthnetException extends Exception {}
...
class pmAuthnet {
    ...
    public function __construct($apilogin = '', $transkey = '', $sandbox = false) {
        $apilogin = trim($apilogin);
        $transkey = trim($transkey);
        $this->sandbox = (bool) $sandbox;
        
        if (empty($apilogin) || empty($transkey)) {
            throw new pmAuthnetException('Missing ' . __CLASS__ . '() login credentials', self::EXCEPTION_CREDENTIALS);
        }
        
        $this->authneturl = ($this->sandbox) ? self::SANDBOX_URL : self::LIVE_URL;
        ...
        $this->setParameter('x_login', $apilogin);
    ...
    public function __toString() {
        ...
        $output = '';
        $output .= '<table summary="pmAuthnet Results" id="authnet">' . "\n";
        ...
        if ($this->results) {
            $output .= '<tr>' . "\n\t\t" . '<th colspan="2"><b>Incomming paramerters</b></th>' . "\n" . '</tr>' . "\n";
    ...
    {
        if (!is_int($retries)) {
            throw new pmAuthnetException(__METHOD__ . '() arg 1 "retries" must be a whole number value: ' . $retries . ' given.', self::EXCEPTION_RETRIES);
        }
        
        $count = 0;
        while ($count < $retries) {
            $this->ch = curl_init($this->authneturl);
        ...
        throw new pmAuthnetException('Connection error: ' . curl_error($this->ch) . ' (' . curl_errno($this->ch) . ')', self::EXCEPTION_CURL);
    }
    ...
    final public function setTransaction($cardnum, $expiration, $amount, $cvv = null, $invoice = null, $tax = null)
    {
        ...
        if (empty($this->params['x_card_num'])) {
            throw new pmAuthnetException('Required information for transaction processing omitted: credit card number', self::EXCEPTION_CREDITCARD);
        }
        
        if (empty($this->params['x_exp_date'])) {
            throw new pmAuthnetException('Required information for transaction processing omitted: expiration date', self::EXCEPTION_EXPIRATION);
        }
        
        if (empty($this->params['x_amount'])) {
            throw new pmAuthnetException('Required information for transaction processing omitted: dollar amount', self::EXCEPTION_AMOUNT);
        }
        
        if (!$this->validateExpirationDate()) {
            throw new pmAuthnetException('Expiration date is in an invalid format', self::EXCEPTION_EXPIRATION);
        }
    }
    ...
    final public function setParameter($field = '', $value = null)
    {
        ...
        if (!is_string($field)) {
            throw new pmAuthnetException(__METHOD__ . '() arg 1 must be a string: ' . gettype($field) . ' given.', self::EXCEPTION_PARAM_FIELD);
        }
        
        if (!is_string($value) && !is_numeric($value) && !is_bool($value)) {
            throw new pmAuthnetException(__METHOD__ . '() arg 2 (' . $field . ') must be a string, integer, or boolean value: ' . gettype($value) . ' given.', self::EXCEPTION_PARAM_VALUE);
        }
        
        if (empty($field)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a parameter field to be named.', self::EXCEPTION_PARAM_FIELD);
        }
        
        if ($value === '' || is_null($value)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a parameter value to be assigned: "' . $field . '" given.', self::EXCEPTION_PARAM_VALUE);
        }
    ...
    final public function setTransactionType($type = '')
    {
        ...
        if (!in_array($type, $typeArray)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid value to be assigned.', self::EXCEPTION_TRANS_TYPE);
        }
    ...
    final public function setEcheck($aba, $account, $accttype, $bankname, $bankacctname, $echecktype, $transtype = 'AUTH_CAPTURE')
    {
        ...
        if (!preg_match('/^\d{1,4}(\.?\d{0,2})?$/', $amount)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid dollar amount.', self::EXCEPTION_ECHECK_AMOUNT);
        }
        
        if (!preg_match('/^\d{9}$/', $aba)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a nine digit ABA/routing number.', self::EXCEPTION_ECHECK_ROUTING);
        }
        
        if (!preg_match('/^\d{6,20}$/', $account)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid account number.', self::EXCEPTION_ECHECK_ACCOUNT);
        }
        
        if (empty($bankname)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid bank name.', self::EXCEPTION_ECHECK_BANK_NAME);
        } else {
            $bankname = substr($bankname, 0, 50);
        }
        
        if (empty($bankacctname)) {
            throw new pmAuthnetException(__METHOD__ . '() requires the name that appears on the checking account.', self::EXCEPTION_ECHECK_ACCT_NAME);
        }
        
        $accountTypeArray = array('CHECKING', 'BUSINESSCHECKING', 'SAVINGS');
        if (!in_array($accttype, $accountTypeArray)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid bank account type to be assigned.', self::EXCEPTION_ECHECK_ACCT_TYPE);
        }
        
        $echeckTypeArray = array('CCD', 'PPD', 'TEL', 'WEB');
        if (!in_array($echecktype, $echeckTypeArray)) {
            throw new pmAuthnetException(__METHOD__ . '() requires a valid echeck type value to be assigned.', self::EXCEPTION_ECHECK_TYPE);
        }
        
        $transTypeArray = array('AUTH_CAPTURE', 'CREDIT');
        if (!in_array($transtype, $transTypeArray)){
            throw new pmAuthnetException(__METHOD__ . '() requires a valid transaction type.', self::EXCEPTION_ECHECK_TRANS_TYPE);
        }
